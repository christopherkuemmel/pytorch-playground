...
# Initialize table with all zeros (state_space, action_space)
Q = np.zeros([game.max_row * game.max_col, len(actions)])
...
for i in range(num_episodes):
    ...
    while j < 99:
        ...
        a = np.argmax(Q[s, :] + np.random.randn(1, len(actions)) * (1. / (i + 1)))
